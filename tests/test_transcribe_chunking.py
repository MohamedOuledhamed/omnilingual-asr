...
def test_transcribe_fallback_timestamps_when_align_fails(monkeypatch):
    _install_fake_fairseq(monkeypatch)
    ...
    def failing_align(model, wav_segment, sr, text):
        raise RuntimeError("boom")

    monkeypatch.setattr(pipeline_mod, "align_ctc", failing_align)
    ...
    pipe._create_batch_simple = types.MethodType(fake_batch, pipe)
    pipe._apply_model = types.MethodType(fake_apply, pipe)
    pipe._fallback_timestamps = types.MethodType(
        pipeline_mod.ASRInferencePipeline._fallback_timestamps, pipe
    )
